...
export default function HomePage() {
  ...
  useEffect(() => {
    // Clear any existing session on the check-in screen
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2rem 1rem' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '2.5rem' }} className="fade-up">
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Strength & Conditioning</p>
      </div>
      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        {!selected ? (
          <div className="fade-up">
            ...
            <input className="input" placeholder="Search your name…" value={search} onChange={e => setSearch(e.target.value)} style={{ marginBottom: '0.75rem' }} autoComplete="off" />
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: 380, overflowY: 'auto' }}>
              ...
              {filtered.map(p => (
                <button key={p.id} onClick={() => { setSelected(p); setPin(''); setError('') }}
                  style={{ background: 'var(--court-surface)', border: '1px solid var(--court-border)', borderRadius: 10, padding: '0.875rem 1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--text-primary)', textAlign: 'left', fontFamily: 'var(--font-body)', transition: 'border-color 0.15s' }}
            ...
            <div style={{ marginTop: '1.5rem', textAlign: 'center' }}>
              <a href="/coach" style={{ color: 'var(--text-muted)', fontSize: '0.8rem', textDecoration: 'none', letterSpacing: '0.05em' }}>Coach Dashboard →</a>
            </div>
          </div>
        ) : (
          <div className="fade-up">
            ...
            {error && (
              <div style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', borderRadius: 8, padding: '0.625rem', marginBottom: '1rem', color: '#f87171', textAlign: 'center', fontSize: '0.9rem' }}>{error}</div>
            )}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem', marginBottom: '1.5rem' }}>
              {keys.map(k => (
                <button key={k} className="pin-key" onClick={() => k === '✓' ? handleCheckin() : handleKey(k)}
            ...
            {loading && <div style={{ textAlign: 'center', color: 'var(--text-muted)' }}>Checking in…</div>}
          </div>
        )}
      </div>
    </div>
