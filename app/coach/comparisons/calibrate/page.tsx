...
export default function CalibratePage() {
  ...
  // Net image positioning — output as direct percentages for copy-paste
  const netLeft   = (n.x / SW * 100).toFixed(1)
  const netTop    = (n.y / SH * 100).toFixed(1)
  const netWidth  = (n.w / SW * 100).toFixed(1)
  const netHeight = (n.h / SH * 100).toFixed(1)

  // Boys net shifts top up by the difference in net heights
  const girlsNetY = yp(88.25)
  const boysNetY  = yp(95.5)
  const netShift  = girlsNetY - boysNetY  // boys net is higher = smaller y
  const netTopBoys = ((n.y - netShift) / SH * 100).toFixed(1)
  const netHBoys   = ((n.h + netShift) / SH * 100).toFixed(1)
...
`  // ── CALIBRATION ── generated by /coach/comparisons/calibrate
  ...
  // Net image — paste these directly, no math needed
  const NET_L='${netLeft}%', NET_T='${netTop}%', NET_W='${netWidth}%', NET_H_GIRLS='${netHeight}%'
  const NET_T_BOYS='${netTopBoys}%', NET_H_BOYS='${netHBoys}%'`
    ...
    <div style={{padding:'1.5rem',maxWidth:820}}>
      ...
      <div ref={wrapRef} onMouseDown={onMouseDown}
        ...
        <canvas ref={fgRef} width={SW} height={SH}
          style={{position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none'}} />      </div>
