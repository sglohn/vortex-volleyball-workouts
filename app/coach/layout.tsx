...
const NAV = [
  ...
  { href: '/coach/teams',     label: 'Teams',           icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="9" cy="7" r="4"/><path d="M3 21v-2a4 4 0 0 1 4-4h4"/><circle cx="17" cy="17" r="4"/><path d="M15 17h4m-2-2v4"/></svg> },
  ...
  { href: '/coach/display',   label: 'TV Display',      icon: <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg> },
]
...
export default function CoachLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex' }}>
      {/* Sidebar */}
      <aside style={{ width: 215, background: 'var(--black)', display: 'flex', flexDirection: 'column', padding: '1.25rem 0.75rem', flexShrink: 0, position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
        {/* Logo */}
        ...
        {/* Sign out */}
        <button onClick={signOut} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: '0.8rem', padding: '0.5rem 0.625rem', borderRadius: 7, transition: 'color 0.15s', marginTop: '0.5rem' }}>
      ...
      {/* Main content area */}
      <main style={{ flex: 1, overflowY: 'auto', minHeight: '100vh', background: 'var(--page-bg)' }}>
        {children}
      </main>
    </div>
  )
