...
type QuickStatus = 'good' | 'sore' | 'injured'
      ...
      {zones.map(z => {
        ...
        return (
          <rect key={z.id} x={z.x} y={z.y} width={z.w} height={z.h} rx={4}
            fill={status ? bg(status) : isHovered ? 'rgba(255,255,255,0.15)' : 'transparent'}
            stroke={status ? sc(status) : isHovered ? 'rgba(255,255,255,0.5)' : 'transparent'}
...
export default function BodyCheckPage() {
  ...
  useEffect(() => {
    ...
    setPlayerName(JSON.parse(s).playerName ?? '')
  }, [router])
  ...
  const flagCount = Object.keys(regions).length
  const canProceed = quickStatus !== null

  // ── STEP 1: Quick status ──
  if (!quickStatus) return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', background: 'var(--page-bg)' }}>
      <div style={{ width: '100%', maxWidth: 380, textAlign: 'center' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.5rem', marginBottom: '0.375rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <button onClick={() => { setQuickStatus('good'); setTimeout(() => submit(), 100) }}
            ...
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.1rem', color: 'var(--success)' }}>I'm Good</div>
              <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Feeling great, ready to train</div>
            </div>
          </button>

          <button onClick={() => setQuickStatus('sore')}
            style={{ padding: '1.25rem', borderRadius: 14, border: '2px solid rgba(250,204,21,0.4)', background: 'rgba(250,204,21,0.06)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '1rem', textAlign: 'left', transition: 'all 0.15s' }}
            onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--yellow)'}
            onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(250,204,21,0.4)'}>
            <span style={{ fontSize: '2rem' }}>😐</span>
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.1rem', color: '#a16207' }}>A bit sore</div>
              <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Some soreness — tap where on the body</div>
            </div>
          </button>

          <button onClick={() => setQuickStatus('injured')}
            style={{ padding: '1.25rem', borderRadius: 14, border: '2px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.05)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '1rem', textAlign: 'left', transition: 'all 0.15s' }}
            onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--danger)'}
            onMouseLeave={e => e.currentTarget.style.borderColor = 'rgba(239,68,68,0.3)'}>
            <span style={{ fontSize: '2rem' }}>🤕</span>
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.1rem', color: 'var(--danger)' }}>Something hurts</div>
              <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Pain or injury — tap where it is</div>
            </div>
          </button>
        </div>
      </div>
  ...
  // ── STEP 2: Body map (if sore or injured) ──
  const defaultStatus: Status = quickStatus === 'injured' ? 'injured' : 'sore'

  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', padding: '1.25rem', background: 'var(--page-bg)', maxWidth: 480, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
        <button onClick={() => setQuickStatus(null)} style={{ background: 'none', border: 'none', color: 'var(--carolina-dark)', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 500 }}>← Back</button>
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.1rem' }}>
            {quickStatus === 'injured' ? '🤕 Where does it hurt?' : '😐 Where are you sore?'}
          </div>
          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            Tap to mark · tap again for injured · tap once more to clear
          </div>
        </div>
      </div>

      {/* Front/Back toggle */}
      <div style={{ display: 'flex', gap: '0.375rem', background: 'var(--carolina-light)', padding: '0.2rem', borderRadius: 8, border: '1.5px solid var(--carolina-border)', marginBottom: '0.75rem', width: 'fit-content' }}>
        {(['front','back'] as const).map(s => (
          <button key={s} onClick={() => setSide(s)}
            style={{ padding: '0.35rem 1rem', borderRadius: 6, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.82rem', background: side === s ? 'var(--carolina)' : 'transparent', color: side === s ? '#fff' : 'var(--carolina-dark)' }}>
            {s === 'front' ? 'Front' : 'Back'}
          </button>
        ))}
      </div>
      ...
      {flagCount > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem', marginBottom: '0.75rem' }}>
          {Object.entries(regions).map(([id, status]) => {
            ...
            return (
              <div key={id} style={{ fontSize: '0.72rem', fontWeight: 600, padding: '0.2rem 0.6rem', borderRadius: 5, background: status === 'injured' ? 'var(--danger-light)' : '#fef9c3', color: status === 'injured' ? 'var(--danger)' : '#a16207', border: `1px solid ${status === 'injured' ? '#fecaca' : '#fde68a'}` }}>
                {zone?.label ?? id} {status === 'injured' ? '🔴' : '🟡'}
              </div>
            )
          })}
        </div>
      ...
      <div style={{ flex: 1 }}>
        <Figure
          ...
          regions={regions} hovered={hovered}
          onTap={id => {
            setRegions(prev => {
              const cur = prev[id]
              if (!cur) return { ...prev, [id]: defaultStatus }
              if (cur === 'sore') return { ...prev, [id]: 'injured' }
              const next = { ...prev }; delete next[id]; return next
            })
          }}
